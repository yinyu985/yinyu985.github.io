...
// Become 函数确保当前 Pod 成为其命名空间内的 Leader。
...
## 为什么基于 ConfigMap 的方案在主机房断电后表现不佳？（另一个Operator没能及时成为Leader）
...
## 背景
Kubernetes 中的 Lease 资源（隶属 coordination.k8s.io API 组）用于节点心跳和领导选举等协调机制，是一个不算核心但也不可忽视的功能。然而，官方文档（[https://kubernetes.io/docs/reference/）对](https://kubernetes.io/docs/reference/%EF%BC%89%E5%AF%B9)Lease 的版本演进记录极为有限：
...
*   **coordination.k8s.io/v1**：
    ...
    *   **说明**：1.14.0 添加 v1 代码并提升为 Beta，1.17.0 正式稳定。
*   **局限性**：
