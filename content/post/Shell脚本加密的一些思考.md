...
今天同事请教我一个问题，说不想让别人看到脚本执行的内容，我一开始脸懵，后来才了解到，她是不想让用户看到脚本的内容（卸载软件的路径），以防软件的文件被人恶意破坏，导致不能正常运行等问题，于是便有的本文。<!--more-->
假设需要加密的脚本长这样
...
加密，联想到之前群里见过的挖矿脚本，黑客在登录别人机器后，会进行一些常规的操作，但是不希望，机主登录时发现，就会把脚本进行加密，或者从互联网的某一处匿名的地址去下载一段加密的脚本。
那么到底是怎么加密的？？?
...
把脚本转成Base64，然后放到某台机器上，在需要执行的机器上下载下来，然后执行。就算有好奇宝宝，看看脚本里面是啥，打开啥也看不懂，当然标题的加密确实该打引号了，毕竟但凡他听说过Base64,就知道怎么解了。

```
curl -Os http://10.31.140.88/file/delete_after_run && cat delete_after_run | base64 -d |bash
```

命令的意思是，从远程地址下载，然后通过`base64 -d`对其进行解密，然后用bash执行。
