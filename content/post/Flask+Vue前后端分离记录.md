...
如题，记录告警平台从原来的layui升级到Vue,并实现Flask+Vue前后端分离，记录前后端三种解决跨域的方式（有点像茴香豆的四种写法？没事，技多不压身）<!--more-->
...
通过axios模块，安装`npm install axios`,在src路径下创建`axios.js`
...
`app.vue`是Vue的程序入口文件，配置axios请求后端
...
`vite.config.js`是vite的配置文件，可以在这里配置一个代理，从而解决跨域问题
